...
#####重定向和管道
#输入重定向<和输出重定向>
输入重定向，是指从文件中读取内容给命令作为执行，箭头的指向必须是由文件到命令
输出重定向，是指将命令的结果写入文件中，箭头的指向必须是由命令到文件
#管道|
管道是将一个命令的结果作为下一个命令的标准输入，不像重定向必须是文件和命令交互，它是命令和命令的交互
```
...
```bash
#-n执行命令执行的时间间隔，-d高亮显示和上次结果不同地方
#grep的-i忽略大小写，-a把二进制文件当作文本处理
